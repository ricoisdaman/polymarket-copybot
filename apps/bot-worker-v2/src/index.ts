...
function evaluateRisk(event: LeaderEvent, quote: QuoteEstimate): RiskDecision {
  ...
  if (config.filters.blockedSlugPrefixes.length > 0 && event.side === "BUY") {
    const slug = String(event.raw?.slug ?? event.raw?.eventSlug ?? "").toLowerCase();
    if (slug && config.filters.blockedSlugPrefixes.some((prefix) => slug.startsWith(prefix))) {
      return { allowed: false, reason: "SLUG_BLOCKED" };
    }
  }
  ...
  if (event.side === "BUY" && Object.keys(config.filters.sportPriceFilters).length > 0) {
    ...
    if (sport) {
      const sportFilter = config.filters.sportPriceFilters[sport];
      if (sportFilter) {
        if (event.price < sportFilter.min || event.price > sportFilter.max) {
          return { allowed: false, reason: "SPORT_PRICE_FILTER" };
        }
      }
    }
  }
...
async function runReconcileChecks(): Promise<void> {
  ...
  // Value positions at their TRUE worth:
  //  - Resolved YES (unclaimed, manually redeemed on Polymarket): shares × $1.00
  //  - Resolved NO: $0 (usually already zeroed by syncPositionsWithOnChain)
  //  - Still open: cost basis (shares × avgPrice) — unchanged behaviour
  // This prevents false drawdown stops when winning markets have been resolved
  // but the USDC hasn't been claimed yet.
  let positionValue = 0;
  let unredeemedUSDC = 0;
  if (openPositions.length > 0) {
    let resolutionMap = new Map<string, number>();
    try {
      resolutionMap = await fetchTokenResolutionValues(openPositions.map((p) => p.tokenId));
    } catch {
      // Gamma API blip — fall back to cost basis for all positions this cycle
    }
    for (const p of openPositions) {
      const rv = resolutionMap.get(p.tokenId);
      if (rv !== undefined) {
        // Resolved: value at actual resolution price (1 = YES, 0 = NO)
        positionValue += Number(p.size) * rv;
        if (rv >= 0.999) {
          unredeemedUSDC += Number(p.size) * 1.0;
        }
      } else {
        // Still open: cost basis (no gain/loss before resolution)
        positionValue += Number(p.size) * Number(p.avgPrice);
      }
    }
  }
  const totalEquity = state.cashUSDC + Number(positionValue.toFixed(4));

  // Persist equity metrics so the dashboard can show true portfolio value
  // (cash + value of all positions including unclaimed YES winnings).
  await Promise.all([
    setRuntimeMetric(prisma, profileId, "bot.unredeemed_usdc", String(Number(unredeemedUSDC.toFixed(4)))),
    setRuntimeMetric(prisma, profileId, "bot.equity_usdc",     String(Number(totalEquity.toFixed(4)))),
  ]);
